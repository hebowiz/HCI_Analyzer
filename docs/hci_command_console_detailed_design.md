...
Categoryは`LE RF PHY Test`、`Controller & Baseband`、
`Informational Parameters`を用意する。
...
`Controller & Baseband`カテゴリへVersionなし・パラメーターなしで配置する。

```text
01 03 0C 00
```

送信時はControllerの状態が失われてstandby状態へ戻ることを確認ダイアログで
通知する。承認された場合だけ送信し、通常の`HCI_Command_Complete`を3秒待つ。
