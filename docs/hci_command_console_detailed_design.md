...
- 入力値はOpcode単位で保持し、異なるversion間では共有しない

初期版のCategoryは`LE RF PHY Test`のみ。
...
入力値はApplication層でOpcodeごとに保持する。
...
- v1/v2/v3/v4はOpcodeが異なるため、それぞれ別に保持する
- アプリ終了後の永続化は行わない
- 「初期値」ボタンを押した場合、そのOpcodeのキャッシュも初期値へ戻す
...
1. 8コマンドの`ConsoleCommandDefinition`を作成
...
1. 対象はReceiver v1～v3、Transmitter v1～v4、Test Endの8コマンド
2. 応答タイムアウトは3.0秒
3. 応答待ち中の追加送信は禁止
4. Hex直接送信は初期版に含めない
5. パラメーター値は同一Opcodeごとにアプリ実行中だけ保持する
