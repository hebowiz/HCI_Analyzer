...
| `vendor/console_definitions.py` | 外部Vendor定義の読込、検証、汎用エンコード |
...
│             [初期値] [HCI_Reset] [HCI_Test_End] [コマンド送信]│
├──────────────────────────────────────────────────────────────┤
│ 送受信ログ                         [すべて▼] [ログクリア]     │
...
`cancelled`として終了させる。
Timeoutは未接続・接続中を問わず変更でき、次の送信へ即時反映する。
応答待ち中だけ変更不可とする。Port選択Comboboxの幅は14文字とする。
...
外部定義読込後は`Vendor Specific`を追加する。
...
- プレビューは未接続でも利用できる
- パラメーター領域が縦にスクロール可能な場合、Canvasまたは内側フレームの
  余白上でマウスホイールを操作しても上下移動できる
- 入力部品固有のマウス操作には干渉しない
...
通常のコマンド選択肢には表示せず、Packet Preview欄の`HCI_Test_End`
クイックボタンから送信する。パラメーターはなく、確認ダイアログも表示しない。
...
通常のコマンド選択肢には表示せず、Packet Preview欄の`HCI_Reset`
クイックボタンから送信する。Versionとパラメーターはなく、確認ダイアログも
表示しない。
...
タイムアウトは接続設定欄で選択した1、2、3秒のいずれかをTransportへ渡す。
初期値は1秒とし、応答待ち中でなければ変更を次の送信へ即時反映する。
...
ログ本文はASCIIだけで表示する。OSやシリアルドライバーの例外は例外種別、
`errno`、`winerror`へ正規化し、残った非ASCII文字は
`[localized message omitted]`へ置換する。
...
Port、Baud、Timeoutも終了時に保存して次回起動時に復元する。
Timeoutが未保存または1、2、3秒以外の場合は初期値の1秒を使用する。
...
既存`HciEventParser`は標準の未対応Opcodeをエラーにする。Vendor Specific
Commandを送信可能にするため、以下を分離する。

- H4/Event構造として正常か
- Opcodeが詳細デコード対象か

OGF `0x3F`の未登録OpcodeはCommand Complete/StatusでもOpcode、Status、
Return Parametersを基本解析する。その他の未登録標準Opcodeは
`UNKNOWN_OPCODE`を維持する。
...
| 切断中の送信 | 要求を拒否 |
| Vendor定義不正 | エラーログ、定義を追加しない |
| 未確認Vendor定義 | 警告ダイアログ、拒否時は追加しない |
...
1. 対象11コマンドの`ConsoleCommandDefinition`を作成
...
1. 対象はReceiver v1～v3、Transmitter v1～v4、Test End、
   Reset、Read Local Supported Commands v1/v2を含む11コマンド
2. 応答タイムアウトは1、2、3秒から選択し、初期値は1秒
3. 応答待ち中の追加送信は禁止
4. Hex直接送信は初期版に含めない
